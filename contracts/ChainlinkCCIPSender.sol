...
contract ProgrammableTokenTransfers is CCIPReceiver, OwnerIsCreator {
    ...
    function sendMessagePayLINK(
        ...
        address _token,
        uint256 _amount
    ) external onlyOwner onlyWhitelistedDestinationChain(_destinationChainSelector) returns (bytes32 messageId) {
        // Create an EVM2AnyMessage struct in memory with necessary information for sending a cross-chain message
        // address(linkToken) means fees are paid in LINK
        Client.EVM2AnyMessage memory evm2AnyMessage =
            _buildCCIPMessage(_receiver, _text, _token, _amount, address(linkToken));
        ...
        emit MessageSent(
            messageId, _destinationChainSelector, _receiver, _text, _token, _amount, address(linkToken), fees
        );
    ...
    {
        return (lastReceivedMessageId, lastReceivedText, lastReceivedTokenAddress, lastReceivedTokenAmount);
    }
